...
CORE_DIFF_STATS = [
    ...
    "pts", "plus_minus", "pace", "win_numeric"
]

def build_full_feature_dataset(game_logs_df: pd.DataFrame, games_df: pd.DataFrame = None) -> pd.DataFrame:
    """
    ...
    # 1. Compute advanced stats (Four Factors, Pace, Ratings) if missing
    ...
    home_ignore_cols = ["game_id", "game_date", "season"]
    away_ignore_cols = ["game_id", "game_date", "season", "opponent_id"]
    ...
    matchups = pd.merge(home_df, away_df, on=["game_id", "game_date", "season"], how="inner")
    ...
    # Targets
    if "home_pts" in matchups.columns and "away_pts" in matchups.columns:
        diff_features["point_margin"] = matchups["home_pts"] - matchups["away_pts"]
        diff_features["total_points"] = matchups["home_pts"] + matchups["away_pts"]
